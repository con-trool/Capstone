...
class WorkflowManager {
    ...
    public function initializeWorkflow($request_id) {
        try {
            ...
            foreach ($required_levels as $level) {
                ...
                $stmt = $this->pdo->prepare("
                    INSERT INTO budget_database_schema.approval_progress (request_id, approval_level, approver_id, status) 
                    VALUES (?, ?, ?, ?)
                ");
                $stmt->execute([
                    $request_id, 
    ...
    public function processApproval($request_id, $approver_id, $action, $comments = '') {
        try {
            ...
            $stmt = $this->pdo->prepare("
                ...
                FROM budget_database_schema.budget_request 
                WHERE request_id = ?
            ");
            $stmt->execute([$request_id]);
            ...
            $stmt = $this->pdo->prepare("
                INSERT INTO budget_database_schema.history (request_id, timestamp, action, account_id) 
                VALUES (?, NOW(), ?, ?)
            ");
    ...
    public function getRequestsForApprover($approver_id) {
        $stmt = $this->pdo->prepare("
            ...
            FROM budget_database_schema.budget_request br
            JOIN budget_database_schema.approval_progress ap ON br.request_id = ap.request_id
    ...
    public function getApprovalHistory($request_id) {
        $stmt = $this->pdo->prepare("
            ...
            LEFT JOIN budget_database_schema.account a ON ap.approver_id = a.id
            WHERE ap.request_id = ?
    ...
    private function getRequiredApprovalLevels($department_code, $amount) {
        ...
        $stmt = $this->pdo->prepare("
            ...
            FROM budget_database_schema.approval_workflow aw
        ...
        foreach ($workflow_rules as $rule) {
            ...
            $stmt = $this->pdo->prepare("
                SELECT id FROM budget_database_schema.account 
                WHERE role = ? 
                AND department_code = ?
                LIMIT 1
            ");
